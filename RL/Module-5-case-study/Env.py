...
class CabDriver():
    ...
    def state_encod_arch2(self, state, action):
        ...
        if (action[0] != 0 or action[1] != 0): # skip if both are zero, i.e., action not taken
    ...
    def requests(self, state):
        ...
        # possible values for each state according to MDP. storing in repective indexes, we can use index based look up
        possible_requests = [2,12,4,7,8]
        location = state[0]
        requests = np.random.poisson(possible_requests[location])
        ...
        # adding [0,0] index
        possible_actions_idx.append(self.action_space.index((0,0)))
